<template>
  <div class="signup">
    ...
    <div v-if="targetStatus" class="modal is-active">
      <div class="modal-background"></div>
      <div class="modal-card">
        <section class="modal-card-body">
          <p class="modal-card-title">{{this.targetName}}さんの残高</p>
          <div class="content">
            <h1>{{this.targetStatus}}</h1>
          </div>
        </section>
        <footer class="modal-card-foot">
          <button @click="targetStatus=null" class="button is-link">Close</button>
        </footer>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getWallet: async function (uid) {
      ...
      if (docSnap.exists()) {
        this.targetStatus = docSnap.data().wallet;
